...
\begin{remark}
The displayed bijection is the standard one for the Kleisli category of a monad
$GF$ arising from an adjunction $F\dashv G$, here $(-\times S)\dashv(S\Rightarrow-)$,
under which $\Kl(GF)(X,Y)\cong\Set(FX,FY)$. So $\Kl(T_S)$ is the \emph{full image}
of the functor $-\times S$. It is \emph{not} the co-Kleisli category of the comonad
$-\times S$, whose hom-sets are $\Set(X\times S,Y)$: for $X=Y=S$ of size $2$ those
number $2^{4}=16$, against $4^{4}=256$ here. Identifying $\Kl(T_S)$ with the full
image is what makes a linear model possible:
...
\begin{enumerate}
  ...
  \item \S3, remark: v2.0 initially glossed $\Kl(T_S)$ as the co-Kleisli category
        of the comonad $-\times S$. That is wrong --- co-Kleisli hom-sets are
        $\Set(X\times S,Y)$, not $\Set(X\times S,Y\times S)$. Corrected to the full
        image of $-\times S$.
